...
def query_services_received():
    ...
    raw_query = '''SELECT * FROM
                ...
                AND GenderMapping.facility_hfr_code = ServiceReceived.facility_hfr_code where facility.uses_cpt_internally is True) AS virtual_table where service_date = '"''' + str(
                current_date) + '''"'
                union
                SELECT *
                FROM
                ...
                INNER JOIN public."Gender" as Gender on Gender.id = GenderMappings.gender_id
                INNER JOIN public."Facility" as facility on facility.id = GenderMappings.facility_id) as GenderMapping on GenderMapping.local_gender = ServiceReceivedItems.gender
                AND GenderMapping.facility_hfr_code = ServiceReceived.facility_hfr_code where facility.uses_cpt_internally is False) AS virtual_table where service_date = '"''' + str(current_date) + '''"' '''
